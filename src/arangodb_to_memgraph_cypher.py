...
tags = nodes_to_cypher(tags, "tags")
services = nodes_to_cypher(services, "services")
depends_and = edges_to_cypher(depends_and, "depends_and")
depends_or = edges_to_cypher(depends_or, "depends_or")


for i, queries in enumerate([tags, services, depends_and, depends_or]):
    print(i)
    for query in tqdm(queries.split("\n")):
        # print(query)
